...
\usepackage{algorithm,algpseudocode}  %format of the algorithm
...
\lstset{ %
	...
	%showtabs=false,        
	columns=fixed,				 %坚决不添加空格使用fullflexible作为参数,如果想要添加空格使用fixed作为参数,不随便添加空格,只在已经有空格的地方添加空格用flexible
	...
	*p1 = "hello";
	...
	process_unique_ptr(std::move(up5));
			...
			\subsubsection{weak\_ptr}\verb|weak_ptr|一般和\verb|shared_ptr|配合使用。它可以指向\verb|shared_ptr|所指向的对象，但是却不增加对象的引用计数。这样就有可能出现\verb|weak_ptr|所指向的对象实际上已经被释放了的情况。因此，\verb|weak_ptr|有一个\verb|lock|函数，尝试取回一个指向对象的\verb|shared_ptr|。
				...
				\verb|weak_ptr|是为配合\verb|shared_ptr|而引入的一种智能指针，\textbf{它更像是}\verb|shared_ptr|\textbf{的一个助手}，而不是一个智能指针。\textbf{它的最大作用在于协助}\verb|shared_ptr|\textbf{工作}，\textbf{像旁观者那样观测资源的使用情况}。\verb|weak_ptr|被设计为与\verb|shared_ptr|协同工作，可以从一个\verb|shared_ptr|或者另外一个\verb|weak_ptr|对象构造，获得资源的观测权限。但\verb|weak_ptr|没有共享资源，它的构造不会引入指针引用技术的增加，同样，\verb|weak_ptr|析构时也不会导致引用计数减少，它只是一个静静的观察着。\verb|weak_ptr|的一个很重要作用是：\textbf{打破循环引用}。
				\begin{lstlisting}
...
\chapter{指针}
	\url{https://www.cnblogs.com/mq0036/p/3382732.html}
	
	\section{指针数组}
		\verb|int *p1[10];|
		
		“\verb|[]|”的优先级比“\verb|*|”要高。\verb|p1| 先与“\verb|[]|”结合，\textbf{构成一个数组的定义}，\verb|数组名为p1|，\verb|int *|修饰的是数组的内容，即\textbf{数组的每个元素}。
		
 		\begin{figure}[H]
 			\centering
 			\includegraphics[scale=0.5]{ptr}
 			\caption{指针数组与数组指针内存分布对比}
 		\end{figure}
 	...
 	\section{数组指针}
 		...
 		在这里“\verb|()|”的优先级比“\verb|[]|”高，“\verb|*|”号和\verb|p2| \textbf{构成一个指针的定义}，\verb|指针变量名为p2|，\verb|int| 修饰的是数组的内容，即数组的每个元素。数组在这里并没有名字，是个\verb|匿名数组|。那现在我们清楚p2 是一个指针，它指向一个包含10 个\verb|int| 类型数据的数组，\textbf{即数组指针}。
...
\section{拷贝构造函数} 拷贝构造函数创建一个新的对象，此对象是另外一个对象的拷贝品
		 ...
		 之前写拷贝构造函数的时候，\textbf{以为参数为引用，不为值传递，仅仅是为了减少一次内存拷贝}。然而今天看到一篇文章发现自己对拷贝构造的参数理解有误。 \textbf{参数为引用，不为值传递是为了防止拷贝构造函数的无限递归，最终导致栈溢出}。
		 
		 \url{https://www.cnblogs.com/zsq1993/p/6110773.html}
...
\chapter{C++ 内存结构}
	...
	A* a = new A();
			...
			此时的a 占用多少空间，0？ 错，是1，为什么呢，是因为需要\textbf{提供一个地址}给\textbf{对象}。
...
\chapter{调用约定}
	\url{https://blog.csdn.net/luoweifu/article/details/52425733}
	
	\verb|__cdecl、__stdcall、__fastcall|
	
	\section{调用约定概述}
		函数的调用约定，顾名思义就是对函数调用的一个约束和规定(规范)，描述了函数参数是怎么传递和由谁清除堆栈的。它决定以下内容：
		\begin{enumerate}
			\item 函数参数的压栈顺序
			\item 由调用者还是被调用者把参数弹出栈
			\item 产生函数修饰名的方法
		\end{enumerate}
	...
	int __stdcall add(int a, int b);
		...
		上面的\verb|__cdecl|和\verb|__stdcall|就是调用约定，其中\verb|__cdecl|是\textbf{C和C++默认的调用约定}.
		
	\section{声明、定义-约定必须相同}	
		在VC++中，\textbf{调用约定是函数类型的一部分}，因此\textbf{函数的声明和定义处调用约定要相同，不能只在声明处有调用约定，而定义处没有或与声明不同}。
		
		\begin{lstlisting}
	int __cdecl add(int a, int b);	// 默认其实就是这种形式
	int __stdcall  add(int a, int b)
	{
	    return a + b;
	}
	...
	\section{函数调用顺序}
		\url{https://blog.csdn.net/zsy2020314/article/details/9429707}
		
		要深入理解函数调用约定，你须要了解函数的调用过程和调用细节。
		
		假设函数A调用函数B，我们称A函数为”调用者”,B函数为“被调用者”。如下面的代码，\verb|ShowResult()|为调用者，\verb|add()|为被调用者。
		\begin{lstlisting}
	int add(int a, int b)
	{
	    return a + b;
	...
	void ShowResult()
	{
		...
		函数调用过程可以这么描述： 
		\begin{enumerate}
			\item 先将调用者（A）的堆栈的基址（ebp）入栈，以保存之前任务的信息。 
			\item 然后将调用者（A）的栈顶指针（esp）的值赋给ebp，作为新的基址（即被调用者B的栈底）。
			\item 然后在这个基址（被调用者B的栈底）上开辟（一般用sub指令）相应的空间用作被调用者B的栈空间。
			\item 函数B返回后，从当前栈帧的ebp即恢复为调用者A的栈顶（esp），使栈顶恢复函数B被调用前的位置；然后调用者A再从恢复后的栈顶可弹出之前的ebp值（可以这么做是因为这个值在函数调用前一步被压入堆栈）。这样，ebp和esp就都恢复了调用函数B前的位置，也就是栈恢复函数B调用前的状态。
		\end{enumerate}
		
		\begin{figure}[H]
			\centering
			\includegraphics[scale=0.5]{functionRef}
			\caption{函数调用过程示例}
		\end{figure}
		
	\section{cdecl}
		参考 c++Interview c++部分题目
	\section{stdcall}
	...
	\section{总结}
		\begin{table}[H]
			\centering
			\caption{cdecl、stadcall、fastcall 总结}
			\begin{tabular}{p{2cm}<{\centering} | p{4cm}<{\centering} |p{5cm}<{\centering}|p{5cm}<{\centering}}
				\toprule
					影响方式 &  \verb|__cdecl| & \verb|__stdcall| & \verb|__fastcall|\\
				\midrule
				...
				\hline
					C编译修饰约定	&\verb|_functionname|&\verb|_functionname@number|&\verb|@functionname@number|\\							
				\bottomrule
			\end{tabular}
		\end{table}
