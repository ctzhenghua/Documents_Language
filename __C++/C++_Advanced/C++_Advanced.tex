...
class __weak_ptr
{
	...
	{ 
		return _M_refcount._M_get_use_count() == 0; 	//shared Count
	}
	...
	对于\verb|weak_ptr|, 其对应的\verb|__weak_count|的\textbf{拷贝构造函数}如下
	\begin{lstlisting}
//************_Sp_counted_base*****************//
void
...
~__weak_count() // nothrow
{
	...
	\begin{itemize}
		\item \verb|__weak_count|相关的赋值拷贝以及析构函数均只会影响到\verb|weak_count|的值，对\verb|use_count|没有影响；当\verb|weak_count|为0时，释放管理对象。也就是说\verb|__weak_ptr|不影响被管理对象的生命周期。同时由于\verb|__weak_ptr|没有像\verb|__shared_ptr|那样实现\verb|*，->|等常见指针相关操作符，\verb|__weak_ptr|不能直接操作被管理对象；
		
		\item \verb|__weak_count|自身间的赋值以及\verb|__shared_count|对\verb|__weak_count|的赋值时，它们都具有同样的指向管理对象的指针；也就是说当多个\verb|__weak_ptr|和\verb|__shared_ptr|指向同一个被管理对象时，它们共享同一个管理对象，这就保证了可以通过\verb|__weak_ptr|可以判断\verb|__shared_ptr|指向的被管理对象是否存在以及获取到被管理对象的指针。
	\end{itemize}
...
int main()
{
	...
	\subsection{实现原理分析}
		...
		其基本思想是通过\verb|M|继承模板类\verb|enable_shared_from_this|，这样对象M内部将会有一个\verb|__weak_ptr|指针\verb|_M_weak_this|，在第一次创建指向\verb|M|的\verb|shared_ptr Ptr|时，\textbf{通过模板特化}，将会初始化\verb|_M_weak_this|;这样\verb|M|内部也会产生一个指向自身的\verb|weak_ptr|，并且该\verb|weak_ptr|内部的管理对象与\verb|Ptr|的管理对象是相同的.
		
		（这可以从\verb|weak_ptr|内部的\verb|_M_assign|函数看出）
		\begin{lstlisting}
...
\section{sharedCount、weakCount}	
	\subsection{count base}
		\begin{lstlisting}
...
_M_add_ref_lock()
{
	...
	while (!__atomic_compare_exchange_n(&_M_use_count, &__count, __count + 1,true, __ATOMIC_ACQ_REL, __ATOMIC_RELAXED));
}
